...
class AAE(tf.keras.Model):
    ...
    def __init__(self, input_size, h_dim, z_dim):
        ...
        self.ae_optimizer = tf.keras.optimizers.Adam(lr=0.001)
        self.dc_optimizer = tf.keras.optimizers.Adam(lr=0.001)
        self.gen_optimizer = tf.keras.optimizers.Adam(lr=0.001)
    ...
    def make_decoder_model(self):
        ...
        x = tf.keras.layers.LeakyReLU()(x)
        x = tf.keras.layers.Dropout(0.5)(x)
        reconstruction = tf.keras.layers.Dense(self.input_size, activation='sigmoid')(x)
    ...
    def train(self, n_epochs, train_set, valid_set):
        ...
        #         gen_optimizer.lr = clr
            ...
            epoch_ae_loss_avg_valid = tf.compat.v2.metrics.Mean(name='Recostruction loss')
            ...
            self.print_status_bar('Epoch :' + str(epoch+1), True,  [epoch_ae_loss_avg, epoch_dc_loss_avg,
                                                        epoch_dc_acc_avg, epoch_gen_loss_avg])
